...
export const PreviewSettlementCycleResponse = zod.object({
  ...
  "kind": zod.enum(['referral_fee', 'ad_pool_contribution', 'perk_obligation', 'coverage_labor']),
  "amount": zod.number(),
...
export const GetSosSettingsResponse = zod.object({
  ...
  "smsDeliveryStatusMode": zod.enum(['callbacks', 'polling', 'none']).optional().describe('How per-message delivery status (delivered\/failed) reaches the app. \"callbacks\" — Twilio StatusCallback webhooks are signature-verified with the raw auth token. \"polling\" — SMS goes through the Replit Twilio connector (auth token withheld), so statuses are reconciled by polling Twilio\'s API. \"none\" — simulated mode, nothing to track.'),
...
export const GetSosTwilioWebhookStatusResponse = zod.object({
  ...
  "voiceStatus": zod.enum(['configured', 'misconfigured', 'no_credentials', 'no_public_url', 'no_number', 'number_not_found', 'error']).describe('Result of the same check for the number\'s \"A call comes in\" (VoiceUrl) webhook — the AI receptionist\'s inbound call entry point. Precondition failures mirror `status`.'),
  "expectedVoiceUrl": zod.string().nullable().describe('The inbound voice webhook URL the app expects Twilio\'s VoiceUrl to point at.'),
  "configuredVoiceUrl": zod.string().nullable().describe('The VoiceUrl currently configured on the number in the Twilio console, when reachable.'),
 ...
 */
export const configureSosTwilioWebhookBodyTargetDefault = `sms`;

export const ConfigureSosTwilioWebhookBody = zod.object({
  "target": zod.enum(['sms', 'voice']).default(configureSosTwilioWebhookBodyTargetDefault).describe('Which webhook on the number to fix — the SMS \"A message comes in\" URL (default) or the voice \"A call comes in\" URL.')
})
 ...
 */
export const SendSosTestSmsBody = zod.object({
  "toNumber": zod.string().nullish().describe('Recipient number; omitted\/empty falls back to the configured default test recipient.')
})

export const SendSosTestSmsResponse = zod.object({
  "smsMode": zod.enum(['live', 'simulated']).describe('Whether the SMS transport was live (Twilio) or simulated at send time.'),
  "defaultRecipient": zod.string().describe('The configured default test recipient number.'),
  "message": zod.object({
  "id": zod.number(),
  "customerId": zod.number().nullish(),
  "customerName": zod.string().nullish(),
 ...
 */
export const RetryFailedSosMessagesResponse = zod.object({
  "retried": zod.number().describe('Number of failed messages re-dispatched (each recorded as a new message row linked via payload.retryOf)'),
  "skipped": zod.number().describe('Number of failed messages left alone because a retry already exists (successful, or itself being retried in this batch)')
})
...
export const ReconcileGustoResponse = zod.object({
  ...
  "unavailable": zod.array(zod.enum(['employees', 'contractors', 'payroll', 'compensation', 'onboarding', 'benefits', 'tax_documents', 'time_tracking', 'time_off', 'scheduling']))
})
