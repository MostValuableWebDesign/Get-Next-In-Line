...
// ── Co-op post-redemption feedback ───────────────────────────────────────────
// One row per co-op perk redemption we asked the customer about (via the
// existing SMS pipeline). Separate from the public sos_reviews system —
// this dataset is redemption-linked and powers the cross-network sentiment
// dashboard. The unique redemption id means a redemption is only ever asked
// about (and answered) once, even if the request sweep re-runs.
export const coopFeedbackTable = pgTable(
  "coop_feedback",
  {
    id: serial("id").primaryKey(),
    partnershipId: integer("partnership_id")
    ...
    tenantId: integer("tenant_id")
      .notNull()
      .references(() => tenantsTable.id, { onDelete: "cascade" }),
    // Customer identity for the SMS round-trip (E.164). Aggregates never
    // expose it across tenants.
    customerPhone: text("customer_phone").notNull(),
    customerName: text("customer_name"),
    // requested | completed
    status: text("status").notNull().default("requested"),
    // 1-5 satisfaction rating (NULL until the reply arrives).
    rating: integer("rating"),
    // NPS-style "would you recommend" (Y/N reply). NULL = not answered.
    wouldRecommend: boolean("would_recommend"),
    feedbackText: text("feedback_text"),
    // Precomputed at ingestion: positive | neutral | negative.
    sentimentLabel: text("sentiment_label"),
    // Sentiment score in [-1, 1] (NULL when there was no text).
    sentimentScore: numeric("sentiment_score", { precision: 4, scale: 3 }),
    // Extracted themes, e.g. ["staff friendliness", "wait time"].
    themes: jsonb("themes").$type<string[]>().notNull().default([]),
    // True when the sentiment came from the AI integration (vs. fallback).
    sentimentUsedAi: boolean("sentiment_used_ai").notNull().default(false),
    requestedAt: timestamp("requested_at").notNull().defaultNow(),
    respondedAt: timestamp("responded_at"),
  },
  (t) => [
    unique("coop_feedback_redemption_uq").on(t.redemptionId),
    index("coop_feedback_tenant_idx").on(t.tenantId, t.requestedAt.desc()),
    index("coop_feedback_partnership_idx").on(t.partnershipId),
    // Inbound reply matching: newest open request for a phone number.
    index("coop_feedback_phone_status_idx").on(t.customerPhone, t.status, t.requestedAt.desc()),
  ]
);

export type CoopFeedback = typeof coopFeedbackTable.$inferSelect;
export type InsertCoopFeedback = typeof coopFeedbackTable.$inferInsert;

// ── Co-op sentiment insight reports ──────────────────────────────────────────
// Weekly ("YYYY-Www") and monthly ("YYYY-MM") insight reports generated by
// the background worker, ranking a tenant's partnerships by customer
// satisfaction and repeat-visit rate with plain-language recommendations.
// The unique (tenant, periodType, periodKey) trio is the idempotency lock —
// a re-run can never produce duplicate reports for a period.
export const coopSentimentReportsTable = pgTable(
  "coop_sentiment_reports",
  {
    id: serial("id").primaryKey(),
    tenantId: integer("tenant_id")
      .notNull()
    ...
    // Per-partnership ranking rows (partnershipId, partnerName, perkTitle,
    // responses, avgRating, nps, repeatVisitRate, sentiment counts).
    rankings: jsonb("rankings").$type<CoopSentimentRankingRow[]>().notNull().default([]),
    // Plain-language recommendation text shown in the dashboard.
    summary: text("summary").notNull(),
    createdAt: timestamp("created_at").notNull().defaultNow(),
  },
  (t) => [
    unique("coop_sentiment_reports_tenant_period_uq").on(t.tenantId, t.periodType, t.periodKey),
    index("coop_sentiment_reports_tenant_idx").on(t.tenantId, t.createdAt.desc()),
  ]
);

export interface CoopSentimentRankingRow {
...
export type CoopSentimentReport = typeof coopSentimentReportsTable.$inferSelect;
