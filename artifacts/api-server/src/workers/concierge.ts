...
export async function runConciergeTick(now: Date = new Date()): Promise<ConciergeTickResult> {
  ...
  return db.transaction(async (tx) => {
    ...
    // lapsed go back to "waiting" so the open slot can be re-offered instead
    ...
    // activate winners, expire ended boosts. Idempotent state transitions.
    const boostResolution = await resolveCoopBoosts(now);
