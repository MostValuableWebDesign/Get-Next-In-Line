...
describe("concierge tick advisory lock", () => {
  it("skips the tick when another connection holds the lock", async () => {
    ...
    try {
      ...
      expect(result).toEqual({ ran: false, campaignBlasts: 0, reaped: 0, reminders: 0, nudges: 0, expiredPerks: 0, perkReminders: 0, coopReports: 0, escalatedDisputes: 0, tierTransitions: 0 });
    } finally {
