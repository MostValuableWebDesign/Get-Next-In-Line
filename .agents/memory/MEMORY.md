...
- Tenant-facing /api/modules intentionally omits `slug` and any brand fields (strict white-label contract) — frontend module lookups must match on `name`; never add slug or partnerBrand back to that response (a partnerBrand exception was tried and reverted July 2026; the DB column remains but must stay null).
