...
- New OutboundMessageKind values must also be added to the message `kind` enum in the OpenAPI spec — the messages list response is zod-parsed, so an unknown kind turns GET /sos/messages into a 400.
