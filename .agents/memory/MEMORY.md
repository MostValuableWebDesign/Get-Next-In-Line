...
- orval: an operation with both path and query params generates a zod `<Op>Params` and a TS type `<Op>Params` that collide on re-export; avoid query params on parameterized paths or rename. Never name a component schema `<OperationId>Body` OR `<OperationId>Response` — orval emits both names itself; api-zod schemas to import in routes are the operation-derived names, not component names.
