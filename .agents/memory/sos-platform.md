...
- AI receptionist intent parsing uses the Replit OpenAI integration when `AI_INTEGRATIONS_OPENAI_*` env vars exist, else a deterministic keyword parser. Setup was blocked on `awaiting_phone_verification` (user must verify phone in Replit before AI integration works).
