...
- Public queue status is capability-protected, not visit-ID-protected: tracking links carry a per-visit high-entropy token only in the URL fragment; the client sends it as `x-check-in-token`, while only its hash is stored. **Why:** serial IDs are enumerable and bearer tokens in paths leak to access logs. **How to apply:** preserve fragment/header transport, never put the raw capability in a request URL or log, and return a neutral public not-found state for missing/wrong tokens.
